...
# Set which stores information for implemented dataset
# Normalisation statistics calculated across training 
# and validation sets using imageStats.py in calcStats.sh
db_info = {
	...
	'pascal_voc': { 'num_classes': 	 21,
					...
					'normalisation': [[0.457,0.441,0.405],
									  [0.267,0.264,0.281]] # Segmentation Dataset
				  },
...
def build_dataset(args, image_set='train', transform=None):
...
class CustomDataset(Dataset):
	...
	def _encode_target(self, target): return target
	...
	def setStage(self, stage): self._stage = stage
...
class CamVid(CustomDataset):
	...
	def __init__(self, root, train_transform = None, train_transformGT = None, 
									val_transform = None, val_transformGT = None):
		...
		self._sampleFiles = { 'train': readImageList('train'),
							  'val': readImageList('val')	 }
...
class KITTI(CustomDataset):
	...
	def __init__(self, root, val_size, 
								train_transform = None, train_transformGT = None, 
										val_transform = None, val_transformGT = None):
		# Store database root directory
		self._root = root
		# Store image transforms
